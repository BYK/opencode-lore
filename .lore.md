...
* **Lore.md merge conflict resolution via keeping all unique entries**: Lore.md changes require git commit inclusion: When \`.lore.md\` has changes from lore system operations, include them in feature commits rather than creating separate commits. The lore system automatically exports knowledge to \`.lore.md\` during operations, so these changes are part of the feature work. Pattern: check git status before committing, include \`.lore.md\` changes with feature commits to maintain coherent history.
