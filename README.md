...
- [LLM Context Ideas](https://github.com/D7x7z49/llm-context-idea) — D7x7z49's research notes on maintaining LLM context windows across long-lived projects. Two ideas adopted: (1) *Temporal Clustering via Relative Existence* — the normalized variance of relative-existence weights (`C_norm`) gives a lightweight, timestamp-only measure of attention imbalance across conversation history, used to inform distillation segmentation boundaries and as a diagnostic signal; (2) *Square Root Boundary* — the heuristic `k/√N` compression ratio (where k = compressed tokens, N = source tokens) provides a computable diagnostic for distillation quality, logged per-segment to detect aggressive compression. The temporal clustering metric also inspired a recency-biased recall list fused via RRF alongside BM25 results.
