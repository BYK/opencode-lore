...
describe("BatchLLMClient", () => {
  ...
  test("default model is used when no explicit model in opts", async () => {
    ...
    expect(fetchCalls).toHaveLength(1);
    expect(fetchCalls[0].body.requests[0].params.model).toBe("claude-sonnet-4-20250514");
    expect(fetchCalls[0].body.requests[0].params.system[0].text).toBe("sys prompt");
    expect(fetchCalls[0].body.requests[0].params.messages[0].content).toBe("user msg");
